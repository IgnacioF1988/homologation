...
def get_cashflows_csv_headers():
    """Return the header line for cashflows.csv."""
    return 'pk2,isin,fecha,flujo_moneda_local,flujo_usd,balance_sheet,moneda_local,yas_yld_flag,override,source,job_id,fetched_at'
...
def is_empty_csv(content, headers):
    """Check if CSV content is empty (only headers or nothing)."""
    if not content:
        return True
    normalized = normalize(content)
    if not normalized:
        return True
    # Check if only headers (with or without trailing newline)
    if normalized == headers or normalized == headers.strip():
        return True
    return False
...
def sync_cashflows():
    """
    Sync cashflows.csv from SHARED to LOCAL, then clear SHARED.

    Flow:
    1. BBG worker writes cashflows to SHARED/cashflows.csv
    2. This sync copies SHARED → LOCAL
    3. This sync clears SHARED (headers only) to prevent reprocessing
    4. bloomberg.routes.js imports LOCAL to SQL
    ...
    headers = get_cashflows_csv_headers()
    ...
    if normalize(src_content) != normalize(dst_content):
        ...
        # This prevents the same cashflows from being copied again
        clear_content = headers + '\n'
        if write_file_safe(src, clear_content, SHARED_CASHFLOWS_LOCK):
            log("[SYNC] Cleared SHARED/cashflows.csv (headers only)")
        else:
            log("[SYNC] WARNING: Failed to clear SHARED/cashflows.csv")


def sync_bond_characteristics():
    """
    Sync bond_characteristics.csv from SHARED to LOCAL, then clear SHARED.

    Flow:
    1. BBG worker writes characteristics to SHARED/bond_characteristics.csv
    2. This sync copies SHARED → LOCAL
    3. This sync clears SHARED (headers only) to prevent reprocessing
    4. bloomberg.routes.js reads LOCAL and calls sp_Complete_BBG_Instrument
    5. bloomberg.routes.js clears LOCAL (headers only)
    """
    ...
    # Skip if SHARED is empty (only headers)
    headers = get_bond_characteristics_csv_headers()
    if is_empty_csv(src_content, headers):
        return
    ...
    if normalize(src_content) != normalize(dst_content):
        ...
        # Clear SHARED after successful copy (headers only)
        clear_content = headers + '\n'
        if write_file_safe(src, clear_content, SHARED_CHARS_LOCK):
            log("[SYNC] Cleared SHARED/bond_characteristics.csv (headers only)")
        else:
