-- =============================================
-- Migration 036: IPA_02 a IPA_05 - Usar tabla temporal global
-- =============================================
...
-- =============================================
-- IPA_03: Renombrar CxC/CxP
-- =============================================
DROP PROCEDURE IF EXISTS [staging].[IPA_03_RenombrarCxCCxP_v2];
GO

CREATE PROCEDURE [staging].[IPA_03_RenombrarCxCCxP_v2]
    @ID_Ejecucion BIGINT,
    @FechaReporte NVARCHAR(10),
    @ID_Fund INT,
    @Portfolio_Geneva NVARCHAR(50) = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            PRINT 'IPA_03_v2 ERROR: Tabla temporal no existe';
            SET @ErrorCount = 1;
            RETURN 3;
        END
        ...
        SET [CXC/CXP?] = CASE
            WHEN InvestDescription LIKE ''%RECEIVABLE%'' THEN ''CXC''
            WHEN InvestDescription LIKE ''%PAYABLE%'' THEN ''CXP''
            ELSE NULL
        END
        WHERE InvestDescription LIKE ''%RECEIVABLE%''
           OR InvestDescription LIKE ''%PAYABLE%'';';

        EXEC sp_executesql @SQL;

        SET @RowsProcessed = @@ROWCOUNT;
        PRINT 'IPA_03_v2 OK: ' + CAST(@RowsProcessed AS VARCHAR(10)) + ' registros renombrados';
        RETURN 0;

    END TRY
    BEGIN CATCH
        SET @ErrorCount = 1;
        PRINT 'IPA_03_v2 ERROR: ' + ERROR_MESSAGE();
        IF ERROR_NUMBER() = 1205 RETURN 2;
        RETURN 3;
    END CATCH
END;
...
-- =============================================
-- IPA_04: Tratamiento Suciedades
-- =============================================
DROP PROCEDURE IF EXISTS [staging].[IPA_04_TratamientoSuciedades_v2];
GO

CREATE PROCEDURE [staging].[IPA_04_TratamientoSuciedades_v2]
    @ID_Ejecucion BIGINT,
    @FechaReporte NVARCHAR(10),
    @ID_Fund INT,
    @Portfolio_Geneva NVARCHAR(50) = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(@TempTableName, 'U') IS NULL
        BEGIN
            PRINT 'IPA_04_v2 ERROR: Tabla temporal no existe';
            SET @ErrorCount = 1;
            RETURN 3;
        END

        -- Eliminar suciedades (registros con MVBook muy cercano a 0)
        SET @SQL = N'
        ...
        SET @RowsProcessed = @@ROWCOUNT;
        PRINT 'IPA_04_v2 OK: ' + CAST(@RowsProcessed AS VARCHAR(10)) + ' suciedades eliminadas';
        RETURN 0;

    END TRY
    BEGIN CATCH
        SET @ErrorCount = 1;
        PRINT 'IPA_04_v2 ERROR: ' + ERROR_MESSAGE();
        IF ERROR_NUMBER() = 1205 RETURN 2;
        RETURN 3;
    END CATCH
END;
...
-- =============================================
-- IPA_05: Eliminar Cajas MTM
-- =============================================
DROP PROCEDURE IF EXISTS [staging].[IPA_05_EliminarCajasMTM_v2];
GO

CREATE PROCEDURE [staging].[IPA_05_EliminarCajasMTM_v2]
    @ID_Ejecucion BIGINT,
    @FechaReporte NVARCHAR(10),
    @ID_Fund INT,
    @Portfolio_Geneva NVARCHAR(50) = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(@TempTableName, 'U') IS NULL
        BEGIN
            PRINT 'IPA_05_v2 ERROR: Tabla temporal no existe';
            SET @ErrorCount = 1;
            RETURN 3;
        END

        -- Eliminar cajas MTM (Mark to Market)
        SET @SQL = N'
        DELETE FROM ' + @TempTableName + '
        WHERE InvestID LIKE ''%MTM%''
           OR InvestDescription LIKE ''%MARK TO MARKET%'';';

        EXEC sp_executesql @SQL;

        SET @RowsProcessed = @@ROWCOUNT;
        PRINT 'IPA_05_v2 OK: ' + CAST(@RowsProcessed AS VARCHAR(10)) + ' cajas MTM eliminadas';
        RETURN 0;

    END TRY
    BEGIN CATCH
        SET @ErrorCount = 1;
        PRINT 'IPA_05_v2 ERROR: ' + ERROR_MESSAGE();
        IF ERROR_NUMBER() = 1205 RETURN 2;
        RETURN 3;
    END CATCH
END;
...
PRINT '✅ Migration 036 completada - IPA_02 a IPA_05 usando tablas temporales';
GO
