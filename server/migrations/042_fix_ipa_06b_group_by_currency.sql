-- =============================================
-- Migration 042: Corregir IPA_06B para agrupar por moneda
-- IPA_Cash debe contener registros AGREGADOS por moneda, no individuales
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Insertar registros AGREGADOS por moneda desde temp table a staging.IPA_Cash
        -- Agrupa todos los registros de Cash por moneda
        -- InvestID = LocalCurrency (codigo de moneda)
        -- InvestDescription = nombre completo de la moneda
        -- ID_Instrumento y id_CURR quedan NULL (sin homologar)
        SET @SQL = N'
        ...
        SELECT
            ...
            LocalCurrency AS InvestID,
            LocalCurrency AS InvestDescription,
            SUM(ISNULL(AI, 0)) AS AI,
            SUM(ISNULL(MVBook, 0)) AS MVBook,
            NULL AS ID_Instrumento,
            NULL AS id_CURR,
        ...
        ELSE
        BEGIN
            PRINT 'IPA_06B_v2 OK: ' + CAST(@RowsProcessed AS VARCHAR(10)) + ' monedas insertadas en staging.IPA_Cash';
        END
...
PRINT '✅ Migration 042 completada - IPA_06B corregido para agrupar por moneda';
GO
