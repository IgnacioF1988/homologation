...
async function importCashflowsFromCSV() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        // Check if override=True - if so, DELETE existing cashflows for this pk2 first
        // This allows recalculating cashflows with a different yas_yld_flag
        const isOverride = cf.override === 'True' || cf.override === 'true';

        if (isOverride) {
          // Delete ALL existing cashflows for this pk2 (recalculating with new yas_yld_flag)
          const deleteResult = await pool.request()
            .input('pk2', sql.NVarChar(50), cf.pk2)
            .query('DELETE FROM metrics.Cashflows WHERE pk2 = @pk2');

          if (deleteResult.rowsAffected[0] > 0) {
            console.log(`[Bloomberg] Override: deleted ${deleteResult.rowsAffected[0]} existing cashflows for ${cf.pk2}`);
          }
        } else {
          // No override - check if this specific cashflow exists (skip duplicates)
          const existsResult = await pool.request()
            .input('pk2', sql.NVarChar(50), cf.pk2)
            .input('fecha', sql.Date, new Date(cf.fecha))
        ...
        // Insert with new yas_yld_flag and override columns
        await pool.request()
          .input('pk2', sql.NVarChar(50), cf.pk2)
          .input('isin', sql.NVarChar(20), cf.isin)
          .input('fecha', sql.Date, new Date(cf.fecha))
          ...
          .input('moneda_local', sql.NVarChar(10), cf.moneda_local)
          .input('yas_yld_flag', sql.NVarChar(10), cf.yas_yld_flag || null)
          .input('override', sql.NVarChar(5), cf.override || null)
          .input('job_id', sql.BigInt, parseInt(cf.job_id) || null)
          .query(`
            INSERT INTO metrics.Cashflows
              (pk2, isin, fecha, flujo_moneda_local, flujo_usd, balance_sheet, moneda_local, yas_yld_flag, override, source, job_id, fetched_at)
            VALUES
              (@pk2, @isin, @fecha, @flujo_moneda_local, @flujo_usd, @balance_sheet, @moneda_local, @yas_yld_flag, @override, 'BBG', @job_id, GETDATE())
          `);
        imported++;
      } catch (err) {
    ...
    if (imported > 0 || skipped > 0) {
      console.log(`[Bloomberg] Cashflows sync complete: ${imported} imported, ${skipped} skipped`);

      // Clear LOCAL cashflows.csv after successful import (headers only)
      const headers = 'pk2,isin,fecha,flujo_moneda_local,flujo_usd,balance_sheet,moneda_local,yas_yld_flag,override,source,job_id,fetched_at\n';
      try {
        await withLock(LOCAL_CASHFLOWS_LOCK, async () => {
          fs.writeFileSync(LOCAL_CASHFLOWS_CSV, headers, 'utf-8');
        });
        console.log('[Bloomberg] Cleared LOCAL/cashflows.csv after import');
      } catch (clearErr) {
        console.error('[Bloomberg] Error clearing LOCAL/cashflows.csv:', clearErr.message);
      }
    }
...
/**
 * Import bond characteristics from CSV and call sp_Complete_BBG_Instrument
 * This enriches colaPendientes.datosOrigen with BBG data (coco, callable, sinkable, yas_yld_flag)
 */
async function importBondCharacteristicsFromCSV() {
  try {
    if (!fs.existsSync(LOCAL_BOND_CHARS_CSV)) {
      return;
    }

    // Check if file was modified
    const stats = fs.statSync(LOCAL_BOND_CHARS_CSV);
    const mtime = stats.mtimeMs;

    if (mtime <= lastBondCharsModified) {
      return; // No changes
    }

    const content = fs.readFileSync(LOCAL_BOND_CHARS_CSV, 'utf-8');
    const lines = content.trim().split('\n');
    ...
    lastBondCharsModified = mtime;
    console.log('[Bloomberg] bond_characteristics.csv changed, processing...');
    ...
    for (let i = 1; i < lines.length; i++) {
      const values = parseCSVLine(lines[i]);
      const char = {};
      headers.forEach((h, idx) => char[h] = values[idx]?.trim() || null);

      if (char.pk2 && char.isin) {
        // Convert boolean-like values to 'S'/'N' format expected by SQL
        const toSN = (val) => {
          if (val === null || val === undefined || val === '') return null;
          const v = String(val).toLowerCase();
          if (v === 'true' || v === '1' || v === 's' || v === 'y' || v === 'yes') return 'S';
          if (v === 'false' || v === '0' || v === 'n' || v === 'no') return 'N';
          return null;
        };
        ...
        characteristics.push({
          pk2: char.pk2,
          coco: toSN(char.coco),
          callable: toSN(char.callable),
          sinkable: toSN(char.sinkable),
          yas_yld_flag: char.yas_yld_flag || null
        });
      }
    }
    ...
    console.log(`[Bloomberg] Found ${characteristics.length} bond characteristics to process`);

    // Call sp_Complete_BBG_Instrument with JSON array
    const pool = await getBBGPool();
    const bbgDataJson = JSON.stringify(characteristics);

    const result = await pool.request()
      .input('bbgDataJson', sql.NVarChar(sql.MAX), bbgDataJson)
      .execute('sandbox.sp_Complete_BBG_Instrument');

    const recordsUpdated = result.recordset[0]?.records_updated || 0;

    if (recordsUpdated > 0) {
      console.log(`[Bloomberg] sp_Complete_BBG_Instrument updated ${recordsUpdated} colaPendientes records`);
    }

    // Clear LOCAL bond_characteristics.csv after successful processing (headers only)
    const headers_line = 'pk2,isin,coco,callable,sinkable,yas_yld_flag,override,job_id,fetched_at\n';
    try {
      await withLock(LOCAL_BOND_CHARS_LOCK, async () => {
        fs.writeFileSync(LOCAL_BOND_CHARS_CSV, headers_line, 'utf-8');
      });
      console.log('[Bloomberg] Cleared LOCAL/bond_characteristics.csv after processing');
    } catch (clearErr) {
      console.error('[Bloomberg] Error clearing LOCAL/bond_characteristics.csv:', clearErr.message);
    }
    ...
    if (err.message && err.message.includes('Could not find stored procedure')) {
      ...
      console.error('[Bloomberg] Error importing bond characteristics:', err.message);
    }
  }
}
